...
public class MenuServiceImpl implements MenuService {
	...
	public int addMenu(JSONObject obj) {
		...
		record2.setName(obj.getString("code"));
		record2.setIsdel(0);
		...
		record.setUrl(obj.getString("uri"));
		record.setIs_menu(Integer.parseInt(obj.getString("type")));
	...
	public int queryMenuCount(JSONObject obj) {
		...
		// 设置查询条件 多个andEqualTo串联表示 and条件查询
		Example.Criteria recordCriteria = example.createCriteria();
		recordCriteria.andEqualTo("isdel", 0).andEqualTo("isdel", 0);
		example.and(recordCriteria);
