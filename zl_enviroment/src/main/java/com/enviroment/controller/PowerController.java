...
import com.enviroment.common.validator.PowerValidator;
...
//			parameter.put("user_id", rsToken.getString("userid"));
			...
			// 数据校验
			ResultErr paramsVal = PowerValidator.queryValidate(parameter);
			if (paramsVal.getErrcode() != 10001) {
				// 非法数据，返回错误码
				return paramsVal;
			}	
			return powerService.result(parameter);
		} catch (Exception e) {
			e.printStackTrace();
			return new ResultErr(ResultStatusCode.ROUTINE_ERR.getCode(), "查询电力数据出现异常");
		}
	}
}
