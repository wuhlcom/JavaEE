...
import com.enviroment.common.validator.WaterValidator;
...
public class WaterController {
	...
	public final static Logger logger = LoggerFactory.getLogger(PowerController.class);
	...
	public Object query(HttpServletRequest request, HttpServletResponse response, @RequestBody String requestBody) {
		try {
			...
			// 数据校验
			ResultErr paramsVal = WaterValidator.queryValidate(parameter);
			if (paramsVal.getErrcode() != 10001) {
				// 非法数据，返回错误码
				return paramsVal;
			}
			return waterService.result(parameter);
		} catch (Exception e) {
			e.printStackTrace();
			return new ResultErr(ResultStatusCode.ROUTINE_ERR.getCode(), "查询水表数据出现异常");
		}
	}
}
