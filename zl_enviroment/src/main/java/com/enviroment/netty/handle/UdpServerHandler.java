package com.enviroment.netty.handle;
...
import com.enviroment.netty.init.StartupEvent;
import com.enviroment.netty.message.MessageParse;
import com.enviroment.repository.redis.RedisRepository;

import java.sql.Timestamp;
import java.util.Date;

/**
 * 接受UDP消息，并保存至redis的list链表中 Created by wj on 2017/8/30.
 *
 */
...
public class UdpServerHandler extends SimpleChannelInboundHandler<DatagramPacket> {
	...
	public void channelRead0(ChannelHandlerContext ctx, DatagramPacket packet) throws Exception {
		...
		if (StringUtils.isNotEmpty(receiveMsg)) {
			...
			// UdpRepository udpRepository = (UdpRepository)
			// StartupEvent.getBean(UdpRepository.class);
			// udpRepository.save(udpRecord);

			// 获取RedirRepository对象
			// RedisRepository redisRepository = (RedisRepository)
			// StartupEvent.getBean(RedisRepository.class);
			...
			MessageParse.classification(receiveMsg);
			// 在这里可以返回一个UDP消息给对方，告知已接收到UDP消息，但考虑到这是UDP消息，此处可以注释掉
			ctx.write(new DatagramPacket(Unpooled.copiedBuffer("QOTM: " + "Got UDP Message!", CharsetUtil.UTF_8),
					packet.sender()));
