...
import com.dazk.db.model.BuildingCalorimeter;
 ...
 */
@Service("buildingCalorimeterInfoDetail")
@Scope(ConfigurableBeanFactory.SCOPE_PROTOTYPE)
public class BuildingCalorimeterInfoDetail extends ExcelReader {
    @Autowired
    private AddDeviceService addDeviceService;
    @Autowired
    private ImportExceptionService importExceptionService;

    private List<BuildingCalorimeter> deviceList = new ArrayList<BuildingCalorimeter>();
    ...
    public void optRow(int sheetIndex, int curRow, List<String> rowList, List<String> title, Map<String, Object> data) throws Exception {
        if (filter(curRow)) {
            ...
            BuildingCalorimeter model = new BuildingCalorimeter();
            ...
            importException.setLine_tag(curRow + 1 + "行,楼栋热表：" + model.getBuilding_unique_code());
...
//				model.setCode(model.getBuilding_code()+model.getUnit_code()+CodeUtil.getTwoDigitCode(houseHolderService.getHotStationId()));
    ...
    private void insert() {
        try {
            for (BuildingCalorimeter model : deviceList) {
                if (StringUtils.isBlank(model.getBuilding_unique_code()) && StringUtils.isBlank(model.getComm_address()) && model.getType() == null) {

                } else {
                    importException = new ImportException();
                    addDeviceService.addBuildingCalorimeterByObj(model);
                }
            }
        } catch (Exception ex) {
            ...
            importException.setImport_type(CodeUtil.IMPORT_DATA_TYPE_BCALORIMETER);
            importExceptionService.insert(importException);
        }
    }

    private void check(BuildingCalorimeter model,List<String> housecodes,List<String> devicecodes) {
        boolean flag = true;
        //校验编号
        ...
        if(model.getPro_type() == null || model.getPro_type().length() > 32){
            flag = false;
            importException.setError_msg(importException.getError_msg() + "品牌最多32字符，请核查后提交！");
        }
