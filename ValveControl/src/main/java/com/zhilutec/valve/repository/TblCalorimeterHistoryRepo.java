...
public interface TblCalorimeterHistoryRepo extends 
	JpaRepository<TblCalorimeterHistory, String>, JpaSpecificationExecutor<TblCalorimeterHistory> {
	...
	List<TblCalorimeterHistory> getPageDataById(@Param("comm_address") String commAddress, 
	...
	@Query(value = "select * from zl_building_calorimeter_data where "
			...
			+ "limit :start,:pages", nativeQuery = true)
	List<TblCalorimeterHistory> getAllData(@Param("startTime") int startTime, @Param("endTime") int endTime,
			@Param("start") int start, @Param("pages") int pages);
