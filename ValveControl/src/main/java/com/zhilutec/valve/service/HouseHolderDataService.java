...
import com.zhilutec.valve.repository.TblHouseHolderDataRepo;
...
public class HouseHolderDataService {
	...
	@Autowired
	private TblHouseHolderDataRepo houseHolderDataRepo;	
	...
	public List<TblValveHouseHolderData> getStealing(StealingCondition condition) throws GlobalErrorException {
		List rs = new ArrayList();
		List<Object[]> stealingList = new ArrayList<>();
		try {
			System.out.println(condition);
			String con1 = condition.getCondition1().toLowerCase();
			String con2 = condition.getCondition2().toLowerCase();
	...
	public List<TblValveHouseHolderData> getStealingGroup(StealingCondition condition) throws GlobalErrorException {
		List rs = new ArrayList();
		List<Object[]> stealingList = new ArrayList<>();
		try {
			...
			if (con1.equals("or") && con2.equals("and")) {
				stealingList = houseHolderDataRepo.findStealingGroupOrAnd(condition.getStart_time(), condition.getEnd_time(),
						condition.getWit_min(), condition.getWit_max(), condition.getWot_min(), condition.getWot_max(),
						condition.getTemdif());
			} else if (con1.equals("and") && con2.equals("or")) {
				stealingList = houseHolderDataRepo.findStealingGroupAndOr(condition.getStart_time(), condition.getEnd_time(),
						condition.getWit_min(), condition.getWit_max(), condition.getWot_min(), condition.getWot_max(),
						condition.getTemdif());
			} else if (con1.equals( "or") && con2.equals("or")) {
				stealingList = houseHolderDataRepo.findStealingGroup2Or(condition.getStart_time(), condition.getEnd_time(),
						condition.getWit_min(), condition.getWit_max(), condition.getWot_min(), condition.getWot_max(),
						condition.getTemdif());
			} else {
