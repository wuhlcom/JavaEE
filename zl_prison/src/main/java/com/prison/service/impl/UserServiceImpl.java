...
import com.prison.common.result.StatusCode;
import com.prison.common.utils.MD5Util;
...
public class UserServiceImpl implements UserService {
	...
	public Result add(JSONObject jsonObj) {
		...
		if (exist >= 1) {
			return Result.error(StatusCode.REPETITION_ERR.getCode(), "用户名已经存在");
		}
		...
		if (exist >= 1) {
			return Result.error(StatusCode.REPETITION_ERR.getCode(), "邮箱已经存在");
		}
		...
		if (tel != null && !tel.isEmpty()) {
			...
			exist = userDao.selectCount(record);
			if (exist >= 1) {
				return Result.error(StatusCode.REPETITION_ERR.getCode(), "电话号码已经存在");
			}
		}
		...
		if (idcard != null && !idcard.isEmpty()) {
			...
			exist = userDao.selectCount(record);
			if (exist >= 1) {
				return Result.error(StatusCode.REPETITION_ERR.getCode(), "身份证已经存在");
			}
		}
		...
		if (rs >= 1) {
			return Result.ok(StatusCode.SUCCESS.getCode(), "添加用户成功");
		} else {
			return Result.error(StatusCode.Failed.getCode(), "添加用户失败");
		}
	}
	...
	public Result update(JSONObject jsonObj) {
		...
		if (username != null) {
			...
			int exist = userDao.selectCount(record);
			if (exist > 1) {
				return Result.error(StatusCode.NODATA_ERR.getCode(), "用户不存在");
			}
		} else if (id != null) {
			record.setId(id);
			record.setIsdel(0);
		...
		if (telephone != null && !telephone.trim().isEmpty()) {
			...
			int exist = userDao.selectCountByExample(paramsExample);
			if (exist >= 1) {
				return Result.error(StatusCode.REPETITION_ERR.getCode(), "电话已经存在");
			}
		}
		...
		try {
			...
			if (id != null) {
				...
				return Result.error(StatusCode.Failed.getCode(), "更新用户时参数错误");
			}
			...
			if (rs >= 1) {
				return Result.ok(StatusCode.SUCCESS.getCode(), "更新用户成功");
			} else {
				return Result.error(StatusCode.Failed.getCode(), "更新用户失败");
			}
		} catch (Exception e) {
			return Result.error(StatusCode.UNKNOW_ERR.getCode(), "更新用户信息出错");
		}
	}
	...
	public Result delete (JSONObject jsonObj){
		...
		try {
			...
			int rs = userDao.updateByExampleSelective(record, example);
			if (rs >= 1) {
				return Result.ok(StatusCode.SUCCESS.getCode(), "删除用户成功");
			} else {
				return Result.error(StatusCode.Failed.getCode(), "删除用户失败");
			}
		} catch (Exception e) {
			return Result.error(StatusCode.UNKNOW_ERR.getCode(), "删除用户出错");
		}
	}
