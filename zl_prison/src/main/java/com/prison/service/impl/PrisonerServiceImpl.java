...
import com.prison.db.dao.PrisonerDao;
...
public class PrisonerServiceImpl implements PrisonerService {
	...
	public Result add(JSONObject jsonObj) {
		...
		if (exist >= 1) {
			return Result.error(StatusCode.REPETITION_ERR.getCode(), "囚徒编码已经存在");
		}
		...
		if (rs >= 1) {
			return Result.ok(StatusCode.SUCCESS.getCode(), "添加囚徒成功");
		} else {
			return Result.error(StatusCode.Failed.getCode(), "添加囚徒失败");
		}
	}
	...
	public Result update(JSONObject jsonObj) {
		...
		Long prisonerId = jsonObj.getLong("id");
	     if (prisonerId != null) {
			...
			int exist = prisonerDao.selectCount(record);
			if (exist > 1) {
				return Result.error(StatusCode.NODATA_ERR.getCode(), "囚徒不存在");
			}
		}
		...
		if (number != null && !number.trim().isEmpty()) {
			...
			if (exist >= 1) {
				return Result.error(StatusCode.REPETITION_ERR.getCode(), "此编号已存在");
			}
		}
		...
		try {
			...
			if (prisonerId != null) {
				...
				return Result.error(StatusCode.Failed.getCode(), "更新囚徒时参数错误");
			}
		
			int rs = prisonerDao.updateByExampleSelective(record, example);
			if (rs >= 1) {
				return Result.ok(StatusCode.SUCCESS.getCode(), "更新囚徒成功");
			} else {
				return Result.error(StatusCode.Failed.getCode(), "更新囚徒失败");
			}
		} catch (Exception e) {
			return Result.error(StatusCode.UNKNOW_ERR.getCode(), "更新囚徒信息出错");
		}
	}
	...
	public Result delete (JSONObject jsonObj){
		...
		try {
			...
			int rs = prisonerDao.updateByExampleSelective(record, example);
			if (rs >= 1) {
				return Result.ok(StatusCode.SUCCESS.getCode(), "删除囚徒信息成功");
			} else {
				return Result.error(StatusCode.Failed.getCode(), "删除囚徒信息失败");
			}
		} catch (Exception e) {
			return Result.error(StatusCode.UNKNOW_ERR.getCode(), "删除囚徒信息出错");
		}
	}
