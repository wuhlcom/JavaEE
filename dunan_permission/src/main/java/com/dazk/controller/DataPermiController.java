...
public class DataPermiController {
	...
	public Object addDataPermi(HttpServletRequest request, HttpServletResponse response,
		...
		try {
			...
			e.printStackTrace();
			return new ResultErr(ResultStatusCode.ROUTINE_ERR.getCode(), "添加数据权限出现异常");
		} finally {
	...
	public Object addDataPermiBatch(HttpServletRequest request, HttpServletResponse response,
		...
		try {
			...
			e.printStackTrace();
			return new ResultErr(ResultStatusCode.ROUTINE_ERR.getCode(), "批量添加数据权限出现异常");
		} finally {
	...
	public Object delDataPermi(HttpServletRequest request, HttpServletResponse response,
		...
		try {
			...
			if (res >= 1) {
				...
				return new ResultErr(ResultStatusCode.ROUTINE_ERR.getCode(), "删除数据权限时程序出错");
			} else if (res == 0) {
			...
			e.printStackTrace();
			return new ResultErr(ResultStatusCode.ROUTINE_ERR.getCode(), "删除数据权限异常");
		}
	}
	...
	public Object updateDataPermi(HttpServletRequest request, HttpServletResponse response,
		...
		try {
			...
			if (res >= 1) {
				...
				return new ResultErr(ResultStatusCode.ROUTINE_ERR.getCode(), "更新数据权限时程序出错");
			} else if (res == 0) {
			...
			e.printStackTrace();
			return new ResultErr(ResultStatusCode.ROUTINE_ERR.getCode(), "更新数据权限时异常");
		}
	}
